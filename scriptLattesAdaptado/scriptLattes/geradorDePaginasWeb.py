...
class GeradorDePaginasWeb:
	...
	def __init__(self, grupo):
		...
		self.gerarPaginasDePatentesRegistros()
	...
	def gerarPaginaPrincipal(self):
		...
		s+='</ul> <h3 id="registroPatente">Registros e Patentes</h3> <ul>'.decode("utf8")
		if self.nPR0>0:
			s+= '<li> <a href="PR0-0'+self.extensaoPagina+'">Patentes</a> '.decode("utf8")+'('+str(self.nPR0)+')'
		if self.nPR1>0:
			s+= '<li> <a href="PR1-0'+self.extensaoPagina+'">Programas de Computador</a> '.decode("utf8")+'('+str(self.nPR1)+')'
		if self.nPR2>0:
			s+= '<li> <a href="PR2-0'+self.extensaoPagina+'">Marcas Registradas</a> '.decode("utf8")+'('+str(self.nPR2)+')'
		if self.nPR3>0:
			s+= '<li> <a href="PR3-0'+self.extensaoPagina+'">Topografias de Circuito Integrado</a> '.decode("utf8")+'('+str(self.nPR3)+')'
		if self.nPR>0:
			s+= '<li> <a href="PR-0'+self.extensaoPagina+'">Total de Patentes e Registros</a> '.decode("utf8")+'('+str(self.nPR)+')'
		else:
			s+= '<i>Nenhum item achado nos currículos Lattes</i>'.decode("utf8")
	...
	def gerarPaginasDePatentesRegistros(self):
		self.nPR0 =0
		self.nPR1 =0
		self.nPR2 =0
		self.nPR3 =0
		...
		if self.grupo.obterParametro('relatorio-incluir_patente'):
			self.nPR0 = self.gerarPaginaDeProducoes(self.grupo.compilador.listaCompletaPatente, "Patentes", "PR0")
		if self.grupo.obterParametro('relatorio-incluir_programa_de_computador'):
			self.nPR1 = self.gerarPaginaDeProducoes(self.grupo.compilador.listaCompletaProgramaDeComputador, "Programas de computador", "PR1")
		if self.grupo.obterParametro('relatorio-incluir_marca_registrada'):
			self.nPR2 = self.gerarPaginaDeProducoes(self.grupo.compilador.listaCompletaMarcaRegistrada, "Marcas registradas", "PR2")
		if self.grupo.obterParametro('relatorio-incluir_topografia_de_circuito_integrado'):
			self.nPR3 = self.gerarPaginaDeProducoes(self.grupo.compilador.listaCompletaTopografiaDeCircuitoIntegrado, "Topografias de circuito Integrado", "PR3")

		# Total de patentes e registros
