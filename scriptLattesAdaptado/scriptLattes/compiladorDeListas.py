...
class CompiladorDeListas:
	...
	matrizPatente = None
	matrizProgramaDeComputador = None
	matrizMarcaRegistrada = None
	matrizTopografiaDeCircuitoIntegrado = None
	...
	def __init__(self, grupo):
		...
		self.listaCompletaProgramaDeComputador = {}
		self.listaCompletaMarcaRegistrada = {}
		self.listaCompletaTopografiaDeCircuitoIntegrado = {}
		...
		for membro in grupo.listaDeMembros:
			...
			self.listaCompletaProgramaDeComputador 		= self.compilarLista(membro.listaProgramaDeComputador, self.listaCompletaProgramaDeComputador)
			self.listaCompletaMarcaRegistrada      		= self.compilarLista(membro.listaMarcaRegistrada, self.listaCompletaMarcaRegistrada)
			self.listaCompletaTopografiaDeCircuitoIntegrado = self.compilarLista(membro.listaTopografiaDeCircuitoIntegrado, self.listaCompletaTopografiaDeCircuitoIntegrado)
		...
		if self.grupo.obterParametro('relatorio-incluir_programa_de_computador'):
			self.listaCompletaPR = self.compilarListasCompletas(self.listaCompletaProgramaDeComputador, self.listaCompletaPR)
		if self.grupo.obterParametro('relatorio-incluir_marca_registrada'):
			self.listaCompletaPR = self.compilarListasCompletas(self.listaCompletaMarcaRegistrada, self.listaCompletaPR)
		if self.grupo.obterParametro('relatorio-incluir_topografia_de_circuito_integrado'):
			self.listaCompletaPR = self.compilarListasCompletas(self.listaCompletaTopografiaDeCircuitoIntegrado, self.listaCompletaPR)
	...
	def criarMatrizesDeColaboracao(self):
		...
		if self.grupo.obterParametro('grafo-incluir_programa_de_computador'):
			self.matrizesProgramaDeComputador = self.criarMatrizes(self.listaCompletaProgramaDeComputador)
		if self.grupo.obterParametro('grafo-incluir_marca_registrada'):
			self.matrizesMarcaRegistrada = self.criarMatrizes(self.listaCompletaMarcaRegistrada)
		if self.grupo.obterParametro('grafo-incluir_topografia_de_circuito_integrado'):
			self.matrizesTopografiaDeCircuitoIntegrado = self.criarMatrizes(self.listaCompletaTopografiaDeCircuitoIntegrado)
	...
	def uniaoDeMatrizesDeColaboracao(self):
		...
		if self.grupo.obterParametro('grafo-incluir_programa_de_computador'):
			matriz1 += self.matrizesProgramaDeComputador[0]
			matriz2 += self.matrizesProgramaDeComputador[1]
		if self.grupo.obterParametro('grafo-incluir_marca_registrada'):
			matriz1 += self.matrizesMarcaRegistrada[0]
			matriz2 += self.matrizesMarcaRegistrada[1]
		if self.grupo.obterParametro('grafo-incluir_topografia_de_circuito_integrado'):
			matriz1 += self.matrizesTopografiaDeCircuitoIntegrado[0]
			matriz2 += self.matrizesTopografiaDeCircuitoIntegrado[1]
	...
	def imprimirMatrizesDeFrequencia(self):
		...
		print self.matrizProgramaDeComputador
		print "\nMarca registrada"
		print self.matrizMarcaRegistrada
		print "\nTopografia de circuito integrado"
		print self.matrizTopografiaDeCircuitoIntegrado
	...
	def imprimirListasCompletas(self):
		...
		print "\nPrograma de computador"
		self.imprimirListaProducoes(self.listaCompletaProgramaDeComputador)
		print "\nMarca registrada"
		self.imprimirListaProducoes(self.listaCompletaMarcaRegistrada)
		print "\nTopografia de circuito integrado"
		self.imprimirListaProducoes(self.listaCompletaTopografiaDeCircuitoIntegrado)
		print "\nTOTAL DE PR" 
